 ...
 *
 * <p>Validation is split deliberately. {@link #validate()} runs once when the JSON is read
 * and throws on anything structurally wrong, so a bad file fails at load with a message
 * naming the problem. {@link #getItemStack()} never throws: it runs during gameplay, when
 * a wish is granted or a reward handed over, and an id that simply is not registered on
 * this instance (a missing addon, say) must not take a server tick down with it.
 */
@Getter
@Setter
@NoArgsConstructor
public class GenericItemDTO {
	 ...
	 * input. Whether the id resolves against the live registries is deliberately not
	 * checked here — see the class comment.
	 */
