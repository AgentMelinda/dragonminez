 ...
 *
 * <p>Subclassed by splash, lingering and tipped-arrow variants. The potion NBT
 * ({@code Potion} and {@code custom_potion_effects}) is written unconditionally rather
 * than being gated on the item being a {@code PotionItem} — {@code Items.TIPPED_ARROW} is
 * a {@code TippedArrowItem extends ArrowItem}, not a {@code PotionItem}, so such a gate
 * would silently strip every tipped arrow's effects.
 */
@Getter
@Setter
@NoArgsConstructor
