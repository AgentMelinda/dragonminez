 ...
 *
 * <p>Duration and amplifier are stated separately. Previously a potion's effects were a
 * map of id to a single number that was passed to {@code MobEffectInstance(MobEffect, int)}
 * — whose second parameter is the duration — so the amplifier could never be set at all.
 * {@link #fromLegacyEntry} reads those old entries back with that same meaning.
 */
@Getter
@Setter
@NoArgsConstructor
public class PotionEffectDTO {
	...
	/** Reads a pre-2.2 {@code {"<effect id>": <duration>}} entry. */
