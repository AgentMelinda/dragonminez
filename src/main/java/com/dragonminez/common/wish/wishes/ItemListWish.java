...
public class ItemListWish extends Wish {
	...
	/**
	 * Grants a whole armour set. Iterates {@link ArmorItem.Type#values()} rather than the
	 * map's own key set: the sets in {@code MainItems} are {@code HashMap}s keyed by an
	 * enum, whose iteration order follows identity hash codes and so varies between JVM
	 * runs. Datagen output has to be byte-stable, so the order is pinned here.
	 */
