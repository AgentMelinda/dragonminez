...
/**
 * The stock wish lists for Shenron and Porunga, in one place.
 *
 * <p>These used to be written out twice — once in {@code WishManager}, which seeds a new
 * world's {@code <world>/dragonminez/wishes/*.json}, and once in
 * {@code DMZDragonWishProvider}, which generates the shipped datapack. The two copies had
 * already drifted apart, so a player's generated file and the datapack disagreed about
 * what Shenron offers. Both callers now read from here.
 *
 * <p>This lives in {@code common.wish} rather than in the datagen provider so that runtime
 * server code does not have to load a {@code net.minecraft.data.DataProvider} to find out
